...
// Use the same striped lock as the other pre-v7 64-bit operations.
TEXT ·swapIfLessLocked(SB),NOSPLIT,$12-37
	NO_LOCAL_POINTERS
	MOVW	addr+0(FP), R1
	MOVW	R1, 4(R13)
	BL	·lockSwap64(SB)
	// Keep the lock pointer and zero in Store's outgoing argument slots.
	MOVW	8(R13), R0
	MOVW	R0, 4(R13)
	MOVW	$0, R0
	MOVW	R0, 8(R13)
	...
	CMP	R0, R11
	BLO	locked_replace
	BHI	locked_unchanged
	EOR	R8, R2, R11
	AND	R6, R11
	EOR	R8, R4, R0
	AND	R6, R0
	CMP	R0, R11
	BHS	locked_unchanged
locked_replace:
	MOVW	R2, 0(R1)
	MOVW	R3, 4(R1)
	MOVW	$1, R0
